(function () {
  ...
  // build-order-ui.js — shared lazy Mermaid renderer for Build order
  // diagrams. It is injected AFTER the vendored build only when shell.html
  // has at least one .bo-diagram. It used to double as the host for an
  // inline claim-readiness dependency trace; 06 §R09.9 ("no inline
  // dependency map") retired that surface — viewer-runtime.js's
  // renderClaimReadiness no longer emits a .claim-readiness-map anywhere,
  // so the .claim-readiness-map fragments this file's selectors used to
  // carry are gone (LANES.md's L5: "the Mermaid host wiring for
  // .claim-readiness-map, which R09.9 makes dead weight"). This file keeps
  // its full generality for Build order's own diagrams.
  //
