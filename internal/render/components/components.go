...
// RELATIONSHIPS (R09.4's remaining directions, DEPENDS ON /
...
func EdgesHTMLWithCodeLinks(c model.Claim, files []implink.ViewFile, linksGated bool, dependedBy []string, targetStatuses map[string]TargetStatus) template.HTML {
	...
	// The remaining R09.4 direction blocks, built independently of the "extra"
	// facts below so their fixed order — DEPENDS ON, DEPENDED ON BY — never
	// depends on which edges a given claim happens to carry.
	...
	// footer on a claim whose only relationship facts were extras that
	// deliberately do not count toward `links`. R-F.1 fixes
...
// writeRelationshipDirection writes one of R09.4's remaining direction
...
// matching 07a §6's pinned "DEPENDS ON · 1" / "DEPENDED ON BY · 1".
//
