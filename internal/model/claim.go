...
type Claim struct {
	...
	// Edges. rests_on is the required dependency chain and the only
	// remaining claim-to-claim relation the engine walks.
	//
	...
	// byte-identical for that leftover. Nothing walks it.
	//
	// governed_by is gone: KnownFields(true) refuses a file that still
	// carries the key (NIT-29). No leftover field, no alias.
	...
	// matching .k color — are the hand-authored precedent this mirrors).
	// render/components/card.html uses Emphasis to add the claim-card--warn
	// class.
