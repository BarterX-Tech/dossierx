...
func TestLoadClaims_GovernedByIsUnknownField(t *testing.T) {
	dir := t.TempDir()
	p := writeFile(t, dir, "a.yaml", "id: widget.contract.a\nfacet: contract\nmodule: widget\nstatus: draft\nbody: x\ngoverned_by:\n  type: none\n  reason: leftover\n")
	_, err := LoadClaims(dir)
	if err == nil {
		t.Fatal("expected strict-decode error for leftover governed_by, got nil")
	}
	if !strings.Contains(err.Error(), p) {
		t.Errorf("expected error to name the file %q, got: %v", p, err)
	}
	if !strings.Contains(err.Error(), "governed_by") {
		t.Errorf("expected error to name governed_by, got: %v", err)
	}
}
