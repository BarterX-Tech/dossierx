...
// roll-up — it has NO Status branch: it fires on draft AND locked
...
// simultaneously the lock evaluator's refusal reason (a claim cannot lock
...
// lives in the lock evaluator (internal/lock.EvaluateSet); this rule is only the visible, non-blocking
