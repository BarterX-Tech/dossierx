...
| 68 | Lane gate, with the staleness test now the thing that proves step 66 actually happened. | `go build ./... && go vet ./... && go test ./... && gofmt -l $(git ls-files '*.go') && go test ./tests/ -run TestCommittedFixtureViewersAreNotStale -v && git status --porcelain` |
...
| 80 | Cross-lane invariant sweep, each one a property no single lane owns end-to-end. | `! grep -rn "SetEscapeHTML" internal/ cmd/ tests/ && ! grep -rn "dossierx/internal/implink" internal/graph/ && ! grep -rn "has_code_link" internal/ --include='*.go' | grep -v _test.go && ls testdata/fixture-coverage/lint \| wc -l` → `28` `&& go test ./tests/ -run 'TestNoNetworkReferencesAnywhereInEngine|TestCommittedFixtureViewersAreNotStale|TestLintRuleCoverageFixtures' -v && git status --porcelain` (last must be empty) |
