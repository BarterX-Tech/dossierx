...
func assertVersionTranscriptIsRealOutput(t *testing.T, dump siteDump) {
	...
	//
	// THE TRANSFORM IS THE IDENTITY, SINCE v0.5.2, and that is issue #38's fix
	...
	for _, line := range depicted.Lines[1:] {
		...
		t.Errorf("the rendered %q session depicts %q, which `dossierx %s` does not print.\nreal output (from a binary linked the way the release links one, `-X main.version=%s`):\n  %s\nrendered:\n  %s\n"+
			"The page may abridge that output and may not invent a line of it. If the difference is the version's leading `v`: since v0.5.2 there is no difference to reconcile — the release build stamps the tag as tagged, so the tag and the printed version are both %s. A page depicting the stripped form is depicting output no install path produces.",
