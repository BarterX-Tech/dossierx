...
| upgrade the binary: a corpus you did not touch refuses (`governed_by`, `build_role`, `layout_legacy`, pre-ledger), or re-export skills | `dossierx-upgrading` |
...
2. Propose `project.config.yaml` (title, modules, and the engine-fixed `facets: [contract, internals]`)
   and `claims/`. Propose `constitution.yaml` beside it (`dossierx-constitution`); once read,
   **they** approve its lock — `dossierx constitution lock --reason "<their words>"`.
...
| `build-order`, `build_role`, `governed_by`, the doctrine hub | gone: `dossierx-upgrading` folds a corpus that still carries them |
