...
| Freely | author, edit, restructure, delete **draft** claims; reply to any thread; resolve/reopen/edit/delete a thread or reply **you (`--as agent`) authored**; run `dossierx check` as often as you like | read anything; comment on any card; resolve/reopen/edit/delete **any** thread or reply |
| Never | change a **locked** claim without their recorded approval; lock/unlock/flag/reaudit unasked; resolve, reopen, edit or delete a thread or reply a **human** authored | — |
...
| `integrity_failed` | 1 | **read `data.ledger_findings` and branch on `rule`** — one code, several causes, and one of them does NOT mean tampering: `lock-ledger-pre-ledger` is a project whose lock store predates the lock ledger and that still holds something locked (see The pre-ledger crossing below) — it is SILENT on a pre-ledger project holding nothing locked, which crosses correctly on its next lock. Everything else — `lock-ledger-missing`, `lock-ledger-deleted`, `lock-content-drift`, `lock-ledger-released`, `lock-ledger-orphan`, `lock-ledger-abandoned`, `lock-ledger-absent`, `lock-ledger-downgraded`, `comment-ledger-drift`, the `comment-digest-*` and `build-order-*` families — is a locked artifact moved outside the approval path: **do not re-lock to make it go away**, restore the file from git or unlock → fix → lock. Three of them are RESTORE-ONLY: `lock-ledger-deleted`, `comment-digest-unrecorded` and `lock-ledger-downgraded`. For these `unlock → fix → lock` is **wrong** because it signs the edit — restore the named store file from version control and nothing else. The first two are refusals on the WRITE path too, so you meet them as a failed `claim lock` and not only as a finding; re-locking was the step that erased each of them, so no command clears either. `lock-ledger-downgraded` is a lock store hand-edited to claim it predates the ledger: a re-lock there records whatever the claims now say as approved, which is the laundering the rule exists to catch, and such a store is never offered the pre-ledger crossing. |
...
| `write_conflict` | 1 | another process (often `dossierx serve`) holds the lock. Retry. If the retry stalls the same ~10s and fails identically, **read the message before you delete anything — as of v0.5.2 there are two of these and they do not share a recovery. This is the ONE place this bundle asks you to branch on prose, and it is a known defect, not an exception to the envelope rule above (`code` is a promise, prose is not): both states share one `error.code` with nothing under `data` to tell them apart. Fixing that changes the error surface and is deferred (ROADMAP). Read the message here and nowhere else.** (a) *"another docs process may be holding it; remove the file manually if it was left behind by a crash"* — this fires on a TIMEOUT (`internal/lock/filelock.go:84`), and a timeout does not tell apart the two situations that produce this identical message: a process that died inside the critical section and left the sentinel behind, which no timeout ever clears — and a LIVE process still inside the critical section that simply ran past this attempt's ~10s window. The shipped message hedges ("MAY be holding it") because the code itself cannot tell these apart; do not read "may" as "is not." Retry first — it costs nothing, each retry opens its own fresh ~10s window, and a live-but-slow holder that finishes in between clears the sentinel itself, so the next attempt just succeeds. Only reach for deletion once retries have failed identically several times over AND you have confirmed no other `dossierx` process (in particular `dossierx serve`) is actually running against this project — the message names the file (`.dossierx-claims.lock`, or the `.lock` beside whichever store it names), but confirming it is idle is a judgment call the code does not make for you and nothing in it establishes deletion as safe on a bare timeout. If you cannot confirm no process is running, this is branch (b): report it to the human with the path, and stop. (b) *"every attempt failed with … rather than 'already exists' … check the directory's permissions"* — the path could not be opened at all for the whole window. **Do not delete anything**: there may be no sentinel to delete, and if a live process IS inside the critical section, deleting its lock is the one action that corrupts what the lock protects. This is a permissions or scanner problem on the directory — report it to the human with the path, and stop. Do not loop on either. |
...
stands behind; (4) **the build orders AGAIN** — `build-order propose` then `build-order lock` per
module. Step (1) RELEASED each record as it rewrote the artifact, so stopping at (3) discards every
approved build order silently. Commit the rewritten `.build-order.<module>.json` too.
The first lock in a project holding nothing locked crosses the store. Nothing is grandfathered: there is no
`dossierx migrate` and no automatic adoption, because nothing can attest to content no ledger
ever recorded. **Not your call** — unlocking everything discards every standing
approval, so show the human the finding, say what it discards, and get a yes, as for `claim lock`. Commit the lock store and the comment digest store the crossing writes. The
...
4. Ask before installing the git pre-commit hook. If yes, fetch
   `https://raw.githubusercontent.com/BarterX-Tech/dossierx/v0.5.2/scripts/install-git-hook.sh`,
...
| `migrate --adopt` | there is no migration command and no automatic adoption. Four steps, in this order, and do not stop at the third: (1) re-propose every locked build order (`dossierx build-order propose --module <m>`) FIRST, because propose needs the module's claims still locked; (2) unlock every locked claim (`dossierx claim unlock <id> --reason "..."`); (3) lock only what you still stand behind — that first lock crosses the store onto the ledger; (4) re-propose AND re-lock every module's build order, because step 1 released each record and stopping before this discards them all silently. Same sequence the binary's own hint now prints, in the same words. It discards every standing approval, so it is the human's call |
