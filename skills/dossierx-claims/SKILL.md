...
- Edges: `rests_on` (semantic dependency; the target must exist). A
  `rests_on` target's content changing under a locked claim flags
  `review_pending`. Hub-gating walks `rests_on` only.
- **`rests_on` loops are refused** at ERROR (`cycle`).
...
|---|---|---|
| a baselined dependency's content changed underneath it — `rests_on` | `dossierx check`, from a stored hash | `dossierx claim reaudit <id> --confirm --reason "..."` |
