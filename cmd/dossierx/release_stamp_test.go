...
//
// THE PAIR WAS THE OTHER WAY ROUND UNTIL THE STAMP MOVED TO `{{.Tag}}`, and the
// reason it turned over is issue #38. The archive stamped `{{.Version}}` and printed `0.5.1`, while
	...
	for _, stamp := range gateReleaseStamps {
		...
		if stamp.symbol == gateVersionSymbol && values[0] == gateStrippedTemplate {
			t.Fatalf("%s no longer stamps %q, and it names %s — the template that STRIPS the leading `v`. "+
				"That is the `{{.Tag}}` fix run backwards, and it re-opens issue #38: the published archive would print `dossierx version <x.y.z>` while `go install …@v<x.y.z>` prints `v<x.y.z>`, because the module proxy hands debug.ReadBuildInfo the tag verbatim and no ldflags reach that path at all. "+
