...
func TestCheckParity_DraftClaims(t *testing.T) {
	...
	want := "[warning] orphan: widget.contract.one: claim has no rests_on edges in either direction\n" +
		"[warning] orphan: widget.contract.router: claim has no rests_on edges in either direction\n" +
...
func TestCheckParity_ImplinkScanAndStatus(t *testing.T) {
	...
	want := "[warning] orphan: widget.contract.locked: claim has no rests_on edges in either direction\n" +
		...
		"check: OK\n" +
		"impl-links: 1 linked, 0 drifted, 0 partial, 0 unlinked locked claims\n"
	assertCheckParity(t, cfgPath, want, "", false)
}
...
func TestCheckParity_ProjectClaims(t *testing.T) {
	...
	viewer := filepath.Join(root, "build", "viewer", "index.html")
	want := "lint: 0 findings\n" +
		"catalog: wrote " + catalog + " (3 claim(s))\n" +
		"render: wrote " + viewer + "\n" +
		"check: OK\n"
	assertCheckParity(t, cfgPath, want, "", false)
}
