...
type gateBundleSpec struct {
	...
	// byte you needed — so every round produced the same class of finding, about
	// the gate's own material rather than about the release.
	//
	// THEY ARE DELIBERATELY NOT PART OF THE TOTALITY. Handed + Withheld is still
	// exactly the manifest-resolved set, and Referenced is asserted DISJOINT
	// from both. Folding references into Handed would make that equation
	// unenforceable and would quietly reassign ownership: the agent reading a
	// referenced file must report on its own surface, not on the file's.
	Referenced []string
...
func gateBundleAssemble(root string, spec gateBundleSpec) ([]byte, error) {
	...
	// defect one level up: the bundle would hand the bytes over twice, under two
	// headings that say opposite things about whose surface it is. It is refused
	// rather than deduplicated, because the two answers send a maintainer to
	...
	// material already knowing what it is for. Each part carries its Source
	// path, as every part does, which is what lets a finding say that it rests
	// on a document belonging to another surface.
...
//
// The label is the load-bearing part. A referenced document is handed over, like
// the surface's own, and the ONE thing distinguishing them is the heading the
// assembler writes. Without it an agent reviews another surface's file under its
// own name, and because nothing is filtered on the way to the human that finding
// arrives twice — once from the surface that owns the file and once from the
// surface that borrowed it — attributed to the wrong one.
func TestGateBundleReferencedBytesAreHandedOverAndMarkedAsNotYours(t *testing.T) {
	...
	if !strings.Contains(body, "NOT yours to report on") {
		t.Errorf("the referenced document is handed over with no marker separating it from this surface's own documents.\n"+
			"An agent cannot tell borrowed material from its own by looking at the bytes, so it will review it, and a finding about a file this surface does not own arrives under the wrong surface's name — reported twice, since the owning surface's agent reports it too, and nothing is filtered on the way to the human.\nThe bundle reads:\n%s", body)
	}
}

// TestGateBundleRefusesReferencedDocumentsThatAreAlsoTheSurfacesOwn is the
// disjointness half, and it is a refusal rather than a deduplication on purpose.
//
...
// hand over the bytes of a document it told the agent in the same breath that it
// was withholding.
