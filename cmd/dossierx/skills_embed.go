...
func buildAgentGuide(embedded fs.FS) (string, error) {
	...
	names := docNames(docs)
	for _, doc := range docs {
		fmt.Fprintf(&b, "\n---\n\n<a id=\"%s\"></a>\n\n%s", doc.Name, rewriteWikilinks(doc.Body, "", names))
	}
	return b.String(), nil
}
...
func buildAgentsSection(embedded fs.FS) (string, error) {
	...
	b.WriteString(rewriteWikilinks(docs[0].Body, agentGuidePath, docNames(docs)))
...
// rewriteWikilinks turns the bundles' [[skill-name]] cross-references into
// in-document anchors, so a form that has no other files to link to still reads
// as a coherent document instead of showing raw brackets.
//
// names comes from the bundles that were actually loaded, so a link to a bundle
// that does not exist is left as literal [[text]] — visible, and therefore
// fixable — rather than being rewritten into an anchor that goes nowhere. prefix
// is the file the anchors live in ("" when the link target is the same document,
// as in the guide; the guide's path when the body is being inlined somewhere
// else, as in AGENTS.md).
func rewriteWikilinks(body, prefix string, names []string) string {
	out := body
	for _, name := range names {
		out = strings.ReplaceAll(out, "[["+name+"]]", "[`"+name+"`]("+prefix+"#"+name+")")
	}
	return out
}
