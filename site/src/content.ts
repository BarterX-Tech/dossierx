        ...
        releases: [
          ...
          },
          {
            ...
            highlights: [
              ...
              "The boundary, documented rather than implied: TWO changes are not detected, both of them edits that erase BOTH SIDES of a disagreement at once so that no surviving evidence can name it. (1) Repointing claims_dir at a fresh directory AND removing the lock ledger in the SAME change leaves nothing in scope to judge — alone, repoint gives lock-ledger-abandoned and deleting the ledger gives lock-ledger-absent. (2) Deleting a DRAFT claim's comments block AND that claim's key in the comment digest store in the SAME change erases a human's open thread — alone, the block gives comment-ledger-drift and the key gives comment-digest-unrecorded; because an open thread is what blocks claim lock, this one buys the lock, and it is confined to drafts since on a locked claim the same edit is still caught. Each is a conjunction: either half alone is still refused, and check returns ok:true only for the pair. Neither is detected because each resulting tree is indistinguishable from an innocent one — a brand-new project, or an ordinary draft nobody has commented on — and every in-repo source of evidence that would tell them apart is writable by the same person the gate constrains. A build during this release cycle used the parent commit for it and was withdrawn before shipping: --orphan, a rebase or a second config file switched the comparison off without looking unusual, while an ordinary git revert of a lock commit was refused and a genuinely new project in a monorepo was audited against a retired one's ledger. Both are caught by review of a loud diff — a claims_dir line edited beside a deleted tracked store, or a human's own words deleted beside a hand edit of the digest store — which is where a control the committer cannot rewrite actually lives.",
