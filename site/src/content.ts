...
const releases: Release[] = [
        ...
        },
        {
          version: "v0.6.0",
          date: "2026-08-20",
          title: "The skill guides match the engine, and every install prints the tag",
          tag: "Latest release",
          highlights: [
            "SILENT: three shipped skill guides changed, and the skills are embedded in the binary — RE-RUN `dossierx skills export` after upgrading. Anyone who skips the re-export keeps agents following procedures that do not work: a review loop that wedges on a refusal whose recovery is the step scheduled after it, a bootstrap that ends silently uninstructed, and a build-order recovery whose advice fails every claim it touches at the next `dossierx check`. In all three the engine was right and the guide was wrong — no command, flag, error code, lint rule or gate behaves differently.",
            "The three corrections, each a procedure that failed an agent following it as written. The review loop no longer schedules a lock the lock gate refuses: step 4 sent a locked claim through unlock → fix → lock before replying, but the human's thread is still open then, which is exactly what `claim lock` refuses (`unresolved_comments`) — step 4 now ends in the reply, and the lock lands after Resolve. The bootstrap creates the project before exporting the skills: the export resolves its root from `project.config.yaml`, which the old ordering had not written yet — rootless, it exits 0, maintains nothing, and nothing later exported again. And the build-order recovery for a missing `build_role` routes through the approval path — unlock (with the human's `--reason`) → set while draft → lock → re-propose — where the old row could only mean hand-editing a locked file, which fails `integrity_failed` / `lock-content-drift` on every claim so edited at the next `check`.",
            "Both install paths print one version string, the tag as tagged (closes issue #38). The archive stamped `{{.Version}}` — the tag minus its leading `v` — while `go install` falls back to `debug.ReadBuildInfo`, which carries the tag verbatim: one release, two answers (`0.5.1` and `v0.5.1`), so a scripted version-against-tag comparison succeeded one way and failed the other. The stamp is now `{{.Tag}}`, the site's second version constant is deleted, and any strip-the-`v` successor is refused by test.",
            "The release driver now requires the forge to restrict who can create release tags. The gate is enforced by files inside the repository being gated, so anyone with push rights could weaken it and tag that commit — a residual the release procedure could previously only record. The driver reads the rulesets off GitHub's API and refuses the release unless an ACTIVE tag ruleset covers the exact tag and restricts both creation and update; a check that cannot run — no token, a refused token, an uninterpretable pattern — is a refusal naming the problem, never a skip.",
            "The release workflow's tag gate no longer deadlocks against the driver that pushes the tag. It required the tagged commit to be reachable from `origin/main`, but the driver pushes the tag first and `main` last — deliberately, so the site never announces archives that do not exist yet — so the gate refused at the tag push and the driver waited for archives that could never exist; v0.5.1 was finished by hand this way. The gate now asks what the tag can answer alone: the tagged commit is a merge whose tree stamps exactly this release. And every published release page now brackets its generated commit-subject bullets with a pointer to the CHANGELOG entry, where changes your own tooling cannot detect are called out at the top.",
            "The pre-commit hook body moves to v8 — re-run the installer to replace an installed v7. The hook no longer refuses every commit on a path containing `\"`, `\\` or a tab (discovery now uses `-z`, the one output mode git never quotes); printed recoveries name the invocation the reader actually used instead of a path only this repository has; a machine-wide `core.hooksPath` install is said out loud, naming the config file by its real path rather than git's C-quoted rendering of it; the PowerShell wrapper's `Find-Bash` rejects WSL's launcher and now runs under Pester in CI; and `--help` no longer stops mid-sentence.",
            "One engine fix, on Windows: the claims file lock could refuse the one case it exists for. `AcquireFileLock` retried its sentinel only on \"already exists\", but Windows keeps a deleted file's directory entry in a delete-pending state that fails opens with `ERROR_ACCESS_DENIED` — beginning at the exact instant the holder releases, which is the instant a waiter polls. The waiter now polls once more; POSIX keeps failing fast, where the same error is a real permission problem.",
            "`dossierx lock|unlock|flag|reaudit` answer with their replacement, not a bare usage error. The four verbs most likely typed from pre-v0.3.0 memory each returned `unknown command`; each now returns the same `retired_command` refusal as the other retired spellings, with a `run:` hint that actually runs, and the migration table below gains the row naming all four.",
            "In the documents client teams follow, each a procedure that failed a reader following it as written: README's setup paste block installs the CI workflow on both answers to the hook question, not only on \"no\"; README says to commit the comment digest store with the first lock, the moment the engine creates it, not \"once anyone comments\"; and the pre-ledger crossing's final re-propose-and-lock step is conditional on the module being fully locked again — in README, FORMAT.md, the CI template's recovery and the `pre_ledger_unadopted` hint — because the recipe licensed re-locking only what you still stand behind and then handed out a pair that refuses the moment that license is exercised.",
            "A minor bump, not a patch, and deliberately: no schema_version, on-disk store, config or CLI surface change, but two things a consumer sees move — `dossierx version` prints the tag as tagged on every install path, and the embedded skill guides changed, which is the re-export called out above.",
          ],
        },
];
