...
const releases: Release[] = [
        ...
        },
        {
          ...
          highlights: [
            ...
            "SILENT: your committed viewer's bytes change on the next `dossierx check`, with no claim edit and nothing rendering differently. Five maintainer comments in the viewer templates were factually wrong and four of them ship — `graph.css` described a backdrop dim and a drop shadow the pane has never had, both z-index ledgers called the opaque pane root a \"backdrop\", and a `style.css` cross reference named a line number that had drifted. Correcting them changes the generated `index.html` for every project, because that CSS ships inlined and its comments ship with it. The cross-release report classifies all five as silent: no rule, no colour and no layout moved. If you diff your committed viewer after upgrading, this is what you are looking at.",
            "The gate stopped counting findings and started weighing them. This release's own gate ran seven reading rounds and returned 39, 31, 24, 18, 58, 49 and 40 findings across them — never converging, and rising again at round five — because every finding blocked equally, so a stale line number cost the same as a recovery that destroys what it restores. Four mechanisms address that. Prose that restates a number the tree already derives is checked by a test rather than by an agent, starting with the two that had already gone stale inside this release: the retired-command table on this site, which listed twelve of the sixteen commands the binary retires, and the release procedure's own count of where the version pins live. A release freezes the surface manifest at its first reading round, so a curve of findings is measured over a fixed question rather than a growing one. A fix wave is read against its own diff before a full round pays thirteen agents to discover what it broke. And every finding now carries a closed consequence — acts-wrongly, misled or cosmetic — which the recorder crosses with the surface's own reach class to compute a priority P0 to P3; the release stops for P0 and for any P1 nobody has ruled on, and P2 and P3 are written into a tracked ledger the next release reads. None of it relaxes a refusal: coverage is not reduced, a check that cannot run still fails, and every finding still reaches the human. What changed is what a finding costs. Round seven returned 40 findings and 14 of them decided the release.",
            "The one change a reader can see from that work: the retired-command table on the overview page is complete. `lock`, `unlock`, `reaudit` and `flag` were top-level commands until v0.3.0 folded them under the claim noun, the binary has answered all four with their replacement since this release, and that table listed none of them.",
        ...
        surfaces: [
          {
            ...
            can: [
              ...
              "Edit and delete any message — yours or the agent's. Advisory rights run one way: you may act on anything, the agent only on what it wrote itself",
