...
const releases: Release[] = [
        ...
        },
        {
          ...
          highlights: [
            ...
            "SILENT: your committed viewer's bytes change on the next `dossierx check`, with no claim edit and nothing rendering differently. Three maintainer comments inside the inlined stylesheet were factually wrong — `graph.css` described a backdrop dim and a drop shadow the pane has never had — and correcting them changes the generated `index.html` for every project, because that CSS ships inlined. The cross-release report classifies all three as silent: no rule, no colour and no layout moved. If you diff your committed viewer after upgrading, this is what you are looking at.",
            "The gate itself stopped needing four rounds to say the same thing. This release's own gate returned 39, 31, 24 and 18 findings across four reading rounds — decaying without converging — and three mechanisms now address the three reasons. Prose that restates a number the tree already derives is checked by a test rather than by an agent, starting with the two that had already gone stale inside this release: the retired-command table on this site, which listed twelve of the sixteen commands the binary retires, and the release procedure's own count of where the version pins live. A release freezes the surface manifest at its first reading round, so a curve of findings is measured over a fixed question rather than a growing one. And a fix wave is read by two agents against its own diff before a full round pays thirteen to discover what it broke. None of it relaxes a refusal: coverage is not reduced, a check that cannot run still fails, every finding still reaches the human, and any finding at all still fails the verdict.",
            "The one change on this page a reader can see from that work: the migration table below is complete. `lock`, `unlock`, `reaudit` and `flag` were top-level commands until v0.3.0 folded them under the claim noun, the binary has answered all four with their replacement since this release, and this table listed none of them.",
