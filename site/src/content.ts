        ...
        releases: [
          ...
          },
          {
            ...
            highlights: [
              ...
              "The boundary, documented rather than implied: THREE changes are not detected, all of them edits that erase EVERY SIDE of a disagreement at once so that no surviving evidence can name it. (1) THE COLLAPSED SCOPE — repointing claims_dir at a fresh directory AND removing the lock ledger in the SAME change leaves nothing in scope to judge; alone, repoint gives lock-ledger-abandoned and deleting the ledger gives lock-ledger-absent. (2) THE DISOWNED CLAIM, the cheapest of the three — deleting one claim's ledger record, its locked_at stamp AND its hashes baselines from the lock store, flipping its status from locked to draft and rewriting its body, all in the SAME change; no claims_dir edit and no store deleted, and the claim is a freely editable draft again that re-locks with a reason reading like a human's approval. Alone: any key left behind gives lock-ledger-deleted, leaving status locked gives lock-ledger-missing, leaving the record standing gives lock-ledger-orphan, and moving the claim file out of scope without the deletions gives lock-ledger-abandoned — and claim lock refuses it on the write path on the same evidence. (3) THE ERASED REVIEW — deleting a DRAFT claim's comments block AND that claim's key in the comment digest store in the SAME change erases a human's open thread; alone, the block gives comment-ledger-drift and the key gives comment-digest-unrecorded; because an open thread is what blocks claim lock, this one buys the lock, and it is confined to drafts since on a locked claim the same edit is still caught. Each is a conjunction: every half alone is still refused, and check returns ok:true only for the full set. None is detected because each resulting tree is indistinguishable from an innocent one — a brand-new project, a claim that was simply never locked, or an ordinary draft nobody has commented on — and every in-repo source of evidence that would tell them apart is writable by the same person the gate constrains. A build during this release cycle used the parent commit for it and was withdrawn before shipping: --orphan, a rebase or a second config file switched the comparison off without looking unusual, while an ordinary git revert of a lock commit was refused and a genuinely new project in a monorepo was audited against a retired one's ledger. All three are caught by review of a loud diff — each is a hand edit of a tracked JSON store whose whole purpose is to be read in a diff, sitting beside the claim change it was made to permit — which is where a control the committer cannot rewrite actually lives.",
