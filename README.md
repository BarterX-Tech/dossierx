...
| Code links, machine-judged | `check` scans source for `dossierx-claim:` and `dossierx-step:` tags; with `source_dirs` or `source_roots` set, a locked code-producing claim with no link fails, and a stepped claim must be tagged on every step (process-owned steps and `links.mode: none` are the honest exclusions) | `unlinked_claims` |
...
Twenty-five leaf commands under eight nouns. This is a *machine* surface: a human is not expected to run any of it. Use `dossierx <noun> --help` for flags, and `--format text` when you want prose.
...
These are the exception, not the rule. `build/catalog/catalog.json`, `build/viewer/`, and optional `build/conformance/status.json` are *generated* — regenerated in full by every `dossierx check` (the conformance status only when a claim declares an embodiment) — and are safe to `.gitignore`. `build/build-order/<module>.json` starts out in that generated category and leaves it the moment you lock one: a **locked** build order is an approved artifact the gate compares against its record, so commit it like the stores above.
...
| `source_dirs` | []string | no | Directories (relative to the config file) scanned for `dossierx-claim: <id>` and `dossierx-step: <id> #<n> <sha256-hex>` source comments. Each path must stay inside the project. Unset means "do not scan" unless `source_roots` is set; set, plain `check` refuses on a locked code-producing claim with no link (`unlinked_claims`), except `links.mode: none` and process-owned steps. |
| `source_roots` | []{path, repo, ref} | no | Named source trees, including sibling checkouts (`path: ../app`). `path` is relative to the config file and may escape the project; `repo` names the remote; `ref` is the pinned commit, tag or branch and must match `HEAD`. The scan walks `path` and writes `{path, repo, ref, commit}` onto `build/code-links`. Same gate as `source_dirs`. |
