...
1. If the `dossierx` binary is missing, install it with
   `go install github.com/BarterX-Tech/dossierx/cmd/dossierx@v0.5.2`,
...
`--format json` is the default, and every run emits exactly one envelope — with one exception, `serve`, a long-running process that cannot be one envelope and is permanently text-only:
