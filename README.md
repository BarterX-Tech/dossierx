...
|---|---|---|
| Atomic YAML claims | one reviewable fact per file, with a typed edge (`rests_on`) the engine walks and lints | `lint_failed` |
...
Every claim has an `id` (`module.facet.slug`), a `facet`, a `module`, a `status` and a `layout` (`card`, `table`, `list`, `steps`, `tree`, `banner`, `mockup`). Claims name other claims they `rests_on`, forming a graph the engine walks and validates. The full schema is in [FORMAT.md](FORMAT.md). (The `governed_by` block is gone as of v0.7.21; a claim file that still carries one fails to load — see the CHANGELOG for the migration.)
