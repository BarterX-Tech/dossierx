...
| Code links, machine-judged | `check` scans source for `dossierx-claim:` and `dossierx-step:` tags; with `source_dirs` set, a locked code-producing claim with no link fails, and a stepped claim must be tagged on every step (process-owned steps and `links.mode: none` are the honest exclusions) | `unlinked_claims` |
