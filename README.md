...
| `source_dirs` | []string | no | Directories (relative to the config file) scanned for `dossierx-claim: <id>` and `dossierx-step: <id> #<n> <sha256-hex>` source comments — the code side of claim-to-code linking. The step digest is SHA-256 of `" ".join(text.split())` UTF-8; a tag may carry a prefix of at least 8 hex (12 preferred). Unset means "do not scan" and no code-link gate; set, plain `check` refuses on a locked code-producing claim with no link (`unlinked_claims`). |
