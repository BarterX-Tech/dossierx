...
| `build-order-ledger-orphan` | an approved build order with **only its own `locked` flag flipped to `false`**. The two rules above skip an unlocked artifact — correctly, since an unlocked one is a proposal nobody approved — so one boolean removed the file from every rule at once while the approved sequence stayed on disk for an agent to follow. Told apart from an honest re-propose by re-signing the artifact as if the flag were still `true`: if that matches the standing record, the flag is all anyone touched |
