...
Campaign mode prunes one area's whole test surface in one PR: the viewer
fixtures, `tests/procedures`, or one `internal/` package. The value bar,
retention bar, candidate evidence, and validation in [SKILL.md](SKILL.md) apply
to every lane. This file adds the order of work and the lessons of a full
campaign. Each step ends on its completion criterion; do not start the next
step early.

## 1. Baseline

Record the area's test and support line counts and every test file's pass/fail
state at a pinned base SHA (the release or epic branch on a release train).
Keep baseline failures in their own list, and keep environmental failures
(root-owned read-only directories, a shallow clone, a missing browser) separate
from real ones. Upstream, every baseline failure in one full campaign was a
real product bug, not a stale test.
...
prefixes. For DossierX these might be claim schema and ids, lints, locking and
review propagation, comments, manifest and isolation views, the CLI envelope,
viewer render, and skills export. Include the area's cases at shared boundaries
(`tests/`, `tests/procedures`, `viewer-tests`).

Done when every test file and procedure the area owns belongs to exactly one
lane.

## 3. Read-only ledger per lane

Give each lane to its own read-only agent. The agent reads every assigned test
in full, including table rows. It also reads the production owners and their
entry points, callers, history, and CI routing. Each test declaration goes into
a written **ledger** with one mark. A table-driven test is one declaration
...
retiring a window asserted the window was _not_ cleared.
...
several suites replaying the same shared helper through a fake while a stronger
CLI or rendered-output suite already owns the contract. Name the **keeper**
suite for each contract. Prefer the real CLI or rendered-viewer boundary over a
faked collaborator. Correct any ledger errors this pass finds.
...
(`tests/procedures/harness.go`, shared helpers, `testdata/`) through one owner.
With each lane, remove the test-only production seams it unlocks: injection
parameters, getters, reset exports, and indirection layers. Keep CI routing
(`.github/workflows/ci.yml`, `Makefile`) pointing at moved suites. Put durable
test-ownership rules in `AGENTS.md`, drawn from mistakes this campaign actually
found.
...
Campaigns outlive many base commits. Merge the base rather than rebasing a
long, many-commit campaign. When the base modified a file the campaign deleted,
keep the deletion. Port the new contract into the keeper instead, and confirm
every new regression the base added still has a home. Rerun the whole area's
suite on the merged head, with `make viewer-test` when viewer output is in
scope.

Expect review tooling to see a truncated file list on a diff this large. Record
maintainer decisions for compatibility exceptions in the PR evidence rather
than editing gates.
