---
name: test-audit
description: "Invoke whenever writing, changing, reviewing, or sweeping DossierX tests. Authoring gate for new tests plus audit workflow for low-value, implementation-coupled, or duplicative tests and the test-only production seams they demand."
---

# Test Audit

Adapted from OpenClaw's `test-audit` skill (MIT); see [NOTICE.md](NOTICE.md)
for the upstream revision and license.
...
whole area's test surface (for example the viewer fixtures, `tests/procedures`,
or one `internal/` package); before starting one, read
[CAMPAIGN.md](CAMPAIGN.md).

Read `AGENTS.md` first. Its core principles bind every mode here: a skip is a
failure, never narrow coverage silently, and verify what the user sees.
...
3. Why does existing coverage not already catch that failure? Each contract has
   ...
   own distinct risk, such as a CLI envelope or exit code the package test
   cannot reach. Prefer extending a table-driven case or shared fixture over a
   near-duplicate test; consolidate duplicated setup in the same change.
4. Does it need a production seam (exported helper, flag, wrapper, injection
   hook) that no production caller needs? If yes, move the test to the real
   boundary instead.
...
failed proves the fixture, not the fix. One regression at the owner boundary
covers the bug; do not replay the same scenario at every layer it crosses
(package, `tests/`, `tests/procedures`, `viewer-tests`).
...
- exact source, import, or string greps (`os.ReadFile` of a `.go` or `.md`
  file followed by `strings.Contains`);
- private predicate or call-shape tests duplicated at real boundaries;
- duplicate invocations of the same contract;
- per-command replays of shared helpers;
- tests whose only purpose is preserving test-only exports, globals, or wrappers;
- dead production code whose only callers are tests;
- expected values produced by the helper or renderer under test, including
  golden files regenerated from the code they check without review;
- fakes that implement the asserted behavior, or one identical fake standing in
  for different APIs;
- fixtures that supply the state, ordering, or lock the owner should produce,
  or persistence asserted against a file the path never writes;
- capability tests that restate declared flags instead of exercising the
  behavior the flag promises;
- negative controls that pass for an unrelated reason, such as an error from a
  different lint or a rejection the production path never reaches;
- names or fixtures that promise more than the input exercises;
- `t.Skip` paths that turn a missing tool or browser into a pass (AGENTS.md: a
  skip is a failure).
...
routing (`.github/workflows/ci.yml`, `Makefile`), and relevant history. Read
root and scoped `AGENTS.md` files first. When the test claims dependency-backed
...
- engine packages (`internal/`, `cmd/`);
- black-box CLI and contract tests (`tests/`, `tests/procedures/`);
- viewer and browser tests (`viewer-tests/`, rendered fixtures under `testdata/`);
- docs, site, skills, scripts, and release tooling;
...
Keep a test when it independently enforces a public CLI, JSON envelope, exit
code, error code, claim schema, lint, config, migration, storage, security,
platform, default, exported-skill bundle, rendered viewer output, release, or
...
Graph readiness, locking, and review-propagation tests are also governed by the
graph-safety skill; do not weaken them without its proof.
...
1. Run the smallest owner and sibling tests with
   `go test ./<pkg> -run '<Name>' -count=1`.
2. For removed source greps or copied inventories, run the command or dry run
   that owns the real contract (for example `dossierx skills export --check`).
3. Run `gofmt -l` on changed Go files, then `git diff --check`.
4. Run `make test`; run `make viewer-test` when viewer output or
   `viewer-tests/` changed. A skipped browser run is not a pass.
5. Run the repository linter as CI runs it.
6. Inspect `git diff --numstat`; report production/tooling separately from
   tests and test support.

## Landing and continuation

Commit, push, open a PR, or land only when authorized. On a release train, item
PRs target the release or epic branch, not `main`. Land one coherent PR at a
time; after landing, refresh the base and rerun read-only discovery for the
