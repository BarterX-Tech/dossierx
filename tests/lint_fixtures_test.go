...
// Registry) actually fires via the real CLI ("docs lint --json"), not just
...
//
// These fixtures are read-only from "docs lint"'s point of view (lint never
...
// lintFinding mirrors the JSON shape "docs lint --json" encodes
...
// lintFixtureExpectedExit is the exit code "docs lint" must produce for a
...
func TestLintRuleCoverageFixtures(t *testing.T) {
	...
	if len(entries) != 21 {
		t.Fatalf("expected exactly 21 lint fixture directories (one per registered lint rule), found %d: %v", len(entries), entries)
	}
...
// testLintFixtureFiresExactlyOneRule runs "docs lint --json" against
