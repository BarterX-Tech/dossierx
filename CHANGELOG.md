...
any number the tree derives — is not closed by two checks. This entry is the only place that says
so, and it is worth saying: the site's own "26 → 19", the nineteen-under-seven counts and the
surface count are all still hand-typed. A consumer sees
...
reason. It re-reads the `contributing` surface, which declares the manifest in its `reads:` and so
carries it inside its bundle, plus every surface whose own documents or `reads:` list the edit
